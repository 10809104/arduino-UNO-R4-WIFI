...
### 電源供應
- 請確保提供穩定的 電源 供應給 Arduino UNO R4 WiFi 和所有外接設備，避免因電壓不穩或不足導致系統不穩定或無法正常啟動。

### 模組兼容性
- 確保使用的所有模組與 Arduino UNO R4 WiFi 兼容，並且有正確的庫和驅動程式。不同版本的硬體可能會有所差異，請根據需要選擇合適的模組和庫。

### 注意MQTT的client id
- 您必須確保每個連接到 MQTT 伺服器的設備使用唯一的 客戶端 ID，避免兩個設備使用相同的 ID，請確認 MQTT 客戶端的connect()函數
